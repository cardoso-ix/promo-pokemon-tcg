...
test('Autenticação - Credenciais do usuário', () => {
...
test('Autenticação - Token de Sessão HMAC', () => {
...
test('Autenticação - Cookie Builder', () => {
  const token = 'test-token-123';
  const cookie = buildSessionCookie(token);
  assert.ok(cookie.includes('promo_session=test-token-123'));
