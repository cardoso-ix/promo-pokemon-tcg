...
// afiliado do Eduardo. Nada de curadoria: nao olha desconto, loja nem tema. As unicas
// outras edicoes: apagar convite de grupo/canal de terceiro e apagar marca de terceiro
// (@rasgabooster.tcg, #rasgaboot e o que estiver em replica_config.frases_remover).
//
...
for (let i = 0; i < links.length; i++) {
  ...
  if (terminaCom(hostFinal, DOMINIOS_ML)) {
    const afiliado = montarLinkAfiliado(final, apelido, etiqueta);
    texto = trocarTudo(texto, link.url, afiliado);
    convertidos += 1;
    const id = itemIdDe(final);
