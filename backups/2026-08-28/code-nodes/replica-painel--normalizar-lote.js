...
// POST /replica/painel/salvar
// acao: salvar (rota nomeada) | telegram (reconfigurar @) | telegram_off
...
function destinosLimpos(lista) {
  ...
  for (let i = 0; i < lista.length; i++) {
    ...
    if (plataforma === 'whatsapp') {
      identificador = soGrupoWhatsapp(identificador);
      if (!identificador) continue;
    } else {
      identificador = identificador.slice(0, 80);
    }
    ...
    saida.push({
      ...
      nome: String(d.nome || '').trim().slice(0, 80)
    });
  }
  return saida;
}
...
if (acao === 'telegram') {
  if (!telegramId) throw new Error('identificador do Telegram ausente');
}
...
return [{
  json: {
    ...
    nome: nome || 'Nova rota',
    ativo: ativo,
    origens_json: JSON.stringify(origensFiltradas),
    destinos_json: JSON.stringify(destinos),
