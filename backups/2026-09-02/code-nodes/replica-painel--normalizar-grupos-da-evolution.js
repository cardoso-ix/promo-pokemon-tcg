...
// Aceita fetchAllGroups (subject), findChats (name / remoteJid) e canais @newsletter.
...
function ehListavel(chatId) {
  return chatId.indexOf('@g.us') !== -1 || chatId.indexOf('@newsletter') !== -1;
}
...
for (let i = 0; i < bruto.length; i++) {
  ...
  if (!chatId || !ehListavel(chatId)) continue;
...
return [{
  json: {
    grupos: grupos,
    grupos_ok: grupos.length,
    grupos_json: gruposJson,
    grupos_b64: Buffer.from(gruposJson).toString('base64')
  }
}];
