...
ALTER TABLE replica_transmissoes
  ADD COLUMN IF NOT EXISTS opcoes JSONB NOT NULL DEFAULT '{}'::jsonb;

UPDATE replica_transmissoes
   SET opcoes = '{"plataformas":["mercadolivre"],"gerar_imagem":true,"gerar_imagem_preview":false}'::jsonb
 WHERE COALESCE(opcoes, '{}'::jsonb) = '{}'::jsonb;
...
SELECT
  ...
  COALESCE(
    NULLIF((
      SELECT t.opcoes->>'frases_remover'
        FROM replica_transmissao_origens o
        JOIN replica_transmissoes t ON t.id = o.transmissao_id
       WHERE o.chat_id = rota.chat_id AND t.ativo
       ORDER BY t.id
    ...
    (SELECT valor FROM replica_config WHERE chave = 'frases_remover'),
    ''
  ) AS frases_remover,
  COALESCE((
    SELECT t.opcoes
     ...
     ORDER BY t.id
     LIMIT 1
  ), '{}'::jsonb) AS rota_opcoes,
  COALESCE((SELECT valor FROM replica_config WHERE chave = 'formato_post'), '') AS formato_post,
  COALESCE((SELECT valor FROM replica_config WHERE chave = 'atraso_maximo_segundos'), '600') AS atraso_maximo_segundos,
  COALESCE((SELECT valor FROM replica_config WHERE chave = 'limite_legenda_telegram'), '1024') AS limite_legenda_telegram,
  COALESCE((SELECT valor FROM replica_config WHERE chave = 'plataformas'), '["mercadolivre"]') AS plataformas,
  CASE
    WHEN EXISTS (SELECT 1 FROM replica_transmissoes)
    THEN COALESCE((
      SELECT json_agg(d)
